...
export const DEFAULTS = {
  base: process.env.TFP_BASE ?? 'http://localhost:5210',
  viewport: { width: 1440, height: 900 },
  // headless unless TFP_HEADLESS=false (skill exposes a "headed" launch arg).
  headless: process.env.TFP_HEADLESS !== 'false',
  pgContainer: process.env.TFP_PG_CONTAINER ?? 'plai_postgres',
  pgDb: process.env.TFP_PG_DB ?? 'plai',
...
/** Dev-only login — GET /api/dev/login sets the session cookie on the context. */
export async function devLogin(page, base = DEFAULTS.base) {
  await page.goto(`${base}/api/dev/login`, { waitUntil: 'load' });
}
